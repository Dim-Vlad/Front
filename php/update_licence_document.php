...
if ($id <= 0 || $label === '') {
    http_response_code(400); echo json_encode(['error' => 'ID et libellé requis.']); exit;
}
...
$stmt->execute([$id]);
$doc  = $stmt->fetch();
if (!$doc) { http_response_code(404); echo json_encode(['error' => 'Document introuvable.']); exit; }
...
if (isset($_FILES['fichier']) && $_FILES['fichier']['error'] === UPLOAD_ERR_OK) {
    ...
    if ($type !== 'application/pdf') {
        http_response_code(400); echo json_encode(['error' => 'Seuls les fichiers PDF sont acceptés.']); exit;
    }
    $uploadDir = __DIR__ . '/../documents/doc-lience/';
    if (!is_dir($uploadDir)) mkdir($uploadDir, 0755, true);
    $filename = $doc['slug'] . '.pdf';
    if (!move_uploaded_file($tmp, $uploadDir . $filename)) {
        http_response_code(500); echo json_encode(['error' => 'Erreur lors de l\'enregistrement.']); exit;
    }
