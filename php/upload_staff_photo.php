...
if (!in_array($mime, $allowedMimes, true)) {
    echo json_encode(['success'=>false,'error'=>'Format non supporté (JPG, PNG, GIF, WebP)']); exit;
}
...
if (!move_uploaded_file($_FILES['photo']['tmp_name'], $dir . $filename)) {
    echo json_encode(['success'=>false,'error'=>'Erreur lors de l\'enregistrement']); exit;
}

echo json_encode(['success'=>true,'path'=>'/images/staff/' . $filename]);
