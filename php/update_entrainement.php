...
if ($id <= 0 || !in_array($jour, $validJours, true) || $horaire === '' || $t1 === '' || !in_array($colspan, $validColspans, true)) {
    http_response_code(400); echo json_encode(['error' => 'Données incomplètes ou invalides.']); exit;
}

$pdo  = get_pdo();
$check = $pdo->prepare('SELECT id FROM entrainements WHERE id = ?');
$check->execute([$id]);
if (!$check->fetch()) { http_response_code(404); echo json_encode(['error' => 'Créneau introuvable.']); exit; }
