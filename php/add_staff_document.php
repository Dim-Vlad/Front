...
if (!isset($_FILES['fichier']) || $_FILES['fichier']['error'] !== UPLOAD_ERR_OK) {
    echo json_encode(['success'=>false,'error'=>'Fichier PDF manquant']); exit;
}

$mime = mime_content_type($_FILES['fichier']['tmp_name']);
if ($mime !== 'application/pdf') {
    echo json_encode(['success'=>false,'error'=>'Format PDF uniquement']); exit;
}
...
if (!move_uploaded_file($_FILES['fichier']['tmp_name'], $dest)) {
    echo json_encode(['success'=>false,'error'=>'Erreur lors de l\'enregistrement du fichier']); exit;
}
